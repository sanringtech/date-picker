...
Composed Angular DatePicker / DateRangePicker — an input + Angular CDK Overlay popover calendar, assembled on top of the headless [`@sanring/date-picker`](../date-picker/README.md) engine. Ships with sensible default Tailwind styling and CSS Custom Property theming, but nothing here is load-bearing: override the theme tokens, override the `format`, or take full ownership of the source via copy mode.
...
Both modes depend on `@sanring/date-picker` (the engine) as a real npm dependency. Copy mode only transfers ownership of the _shell_ (Input + Overlay + Tailwind markup) — never the state machine, keyboard handling, or WAI-ARIA wiring, which stay inside the engine you still import. This mirrors the shadcn/ui pattern: copying the shell doesn't grant access to engine internals, it just means you now own the shell's source instead of importing it.

## Installation (npm)

```bash
npm install @sanring/date-picker-widget @sanring/date-picker @angular/cdk
```
...
```css
/* styles.css */
@import '@sanring/date-picker-widget/dp-theme.css';
@import 'tailwindcss';
...
import { DatePickerComponent, DateRangePickerComponent } from '@sanring/date-picker-widget';
...
```bash
npm install @sanring/date-picker @angular/cdk date-fns
```

`@sanring/date-picker-widget` itself is **not** a dependency in this mode — you're replacing it with your own copy of its shell.

### 2. Copy these files

From [`projects/date-picker-widget/src/lib/`](./src/lib) in this repo:
...
`CalendarLocale`, `DisabledInput`, `DateRange`, `RangeDayCountLimit` are re-exported from `@sanring/date-picker` — see [that package's README](../date-picker/README.md) for their shapes.
...
- Engine this package builds on: `.claude/prds/date-picker.md`, [`projects/date-picker/README.md`](../date-picker/README.md)
