...
| W5（實際完成 2026-07-26） | Multi 多選 composed 元件 `DateMultiPickerComponent`（selector `sanring-date-multi-picker`）：沿用 engine `CalendarEngine.setSelectionMode('multi')` + `selectedDates` + `removeDate()`/`setSelectedDates()`（M6）；`[(selectedDates)]` 雙向綁定；與 W1/W2 不同——multi 沒有 Draft，每次點選後 overlay 不自動關閉，可連續累積多筆；輸入框支援逗號分隔批次輸入/解析（全部成功才提交，任一筆失敗則整批等待，呼應 Range 的交易式寫入慣例）；每筆已選日期附帶移除 chip（`removeDate()`）+ 清除全部按鈕（`clearSelection()`） | ✅ 11 個單元測試全綠（累積多選、toggle 取消、單筆移除、整批清空、批次輸入解析、disabled 日期擋下、Escape 關閉等）；✅ `ng lint`/`prettier --check`/`ng build date-picker-widget` 全過；已匯出至 `public-api.ts` |
| W6（實際完成 2026-07-26） | Granularity composed 元件 `DateGranularityPickerComponent`（selector `sanring-date-granularity-picker`）：`granularity` input 參數化 Month/Quarter/Year 三種粒度（沿用 engine `GranularityPickerEngine`，M7），共用同一套 `model()` 綁定與 Overlay 整合模式；季度起始月透過 `quarterStartMonth` input 以 DI `useFactory` 延遲注入 `CALENDAR_QUARTER_STARTS_ON`（未提供且 granularity 為 quarter 時丟出明確錯誤，維持 engine 原有 Zero-default 語意）。**範圍縮減（AI 判斷，需 owner 覆核）**：本輪僅實作 Single 選取模式（比照 vue3-datepicker 的 month-picker/year-picker 慣例——「挑一個月/季/年」的單值場景），engine 既有的 Range/Multi 粒度選取語意（M7 範圍內）尚未接上 composed 元件，列為後續追蹤；輸入框為唯讀（僅接受網格點選，不支援自由格式文字輸入） | ✅ 7 個單元測試全綠（12 格月網格選取、4 格季網格＋財年起始月注入、N 格年滑動視窗、disabled 攔截、外部寫值同步、Escape 關閉）；✅ `ng lint`/`prettier --check`/`ng build date-picker-widget` 全過；已匯出至 `public-api.ts` |
| W7（實際完成 2026-07-26） | Time Adjustment composed 元件 `DateTimePickerComponent`（selector `sanring-date-time-picker`）：日期 + 時/分（可選秒）合成完整 `Date`，實作「Widget 套件跨 engine 寫回膠水邏輯」——**此項原本被 engine PRD（`.claude/prds/date-picker.md` M8 驗收門檻）誤勾為已完成，2026-07-26 校正為未完成，本 milestone 是實際補齊出處**；點選日期後開啟時間 Draft（`TimeAdjustmentEngine`，key `'single'`），調整時/分/秒的每一步都透過 `startOrUpdateTimeDraft()` 更新同一筆 Draft，直到使用者按「確認」才呼叫 `confirmTimeDraft()` 合成完整 Date 並寫回 `[(selectedDate)]`（R9 Draft/Confirm 語意，外部 model 在確認前不更新，比照 Range Draft 先例）；「中止」／無 Draft 時的 Escape 會把日網格反白與時間顯示都回滾到最後一次已確認的值（zero-rollback，Decision 3 先例），避免中止後网格仍停留在未確認的暫時點選上。**範圍縮減（AI 判斷，需 owner 覆核）**：本輪僅涵蓋 Single 選取模式＋日期網格輸入框唯讀（自由格式文字輸入與 Range/Multi 搭配時間調整，皆列為後續追蹤，非本 milestone 範圍） | ✅ 8 個單元測試全綠（Draft 不落地模型、確認後合成正確 Date 並關閉 overlay、中止回滾不動 model、disabled 日期不開 Draft、外部寫值同步、Escape 兩種路徑：先中止 Draft／無 Draft 直接關閉）；✅ `ng lint`/`prettier --check`/`ng build date-picker-widget` 全過；已匯出至 `public-api.ts`；新增 `DEFAULT_DATE_TIME_FORMAT_CONFIG`（`yyyy-MM-dd HH:mm`，因為既有 `DEFAULT_DATE_FORMAT_CONFIG` 的純日期 ISO 格式會靜默丟失時間） |

**已知前提**：engine PRD M5（npm 發布）完成、公開穩定 API 定案後，才具備條件供本產出物消費，W0 不預期在 engine M5 完成前啟動。W5-W7 於 2026-07-26 同日新增草案並實際完工——元件命名/選字（selector）為實作時直接拍板（非經完整訪談），且 W6/W7 皆有 AI 自行判斷的範圍縮減（Granularity/Time Adjustment 皆只做 Single 選取模式，Range/Multi 搭配粒度或時間調整不在本輪範圍），兩項縮減標記「需 owner 覆核」，尚未經過使用者確認，狀態比照本 PRD 其餘 AI 草稿章節處理，不可視為已拍板定案。
...
- [x] **2026-07-26 新發現，觸發本輪校正**：第 3 節 vue3-datepicker 比對表當時的判斷前提已過時——`multi-dates`（M6）、Month/Quarter/Year Granularity（M7）、Time Adjustment（M8）三項在 engine 側其實**都已實際完成並發布**（非表格寫成的「未實作」/「治理排除」狀態），只是 widget 層從未組裝對應 composed 元件去消費它們，導致 widget 至今只有 W1 Single、W2 Range 兩種模式，與 engine 現有能力產生明顯落差。→ **已於同日完成 §10 新增的 W5（Multi）/W6（Granularity）/W7（Time Adjustment）**，其中 W7 同時是 engine PRD M8 誤勾項目的實際補齊出處（見 `.claude/prds/date-picker.md` 2026-07-26 校正記錄）。W6/W7 各自帶有「僅 Single 模式」的範圍縮減，待 owner 覆核（見 §10 W6/W7 列與「已知前提」段落）。
