...
status: accepted
owner: jack755051
last_updated: 2026-07-23  # Multi-dates Selection (Decision 11)、Granularity Selection (Decision 12) delta 同步；Programmatic Value Setting (Decision 13)、Range Day-Count Limit (Decision 14) delta 同步；Time-of-Day / Time Adjustment (Decision 15) delta 同步，架構決策由 ADR-0003 拍板（supersede ADR-0002，同日內二次修訂）
related_constitution: .claude/constitutions/date-picker.md
related_adrs: [.claude/adrs/0001-granularity-selection-architecture.md, .claude/adrs/0002-time-adjustment-architecture.md (superseded), .claude/adrs/0003-time-adjustment-engine-decoupling.md]
---
...
- **技術目標**：
  ...
  - 時/分（Time-of-Day）調整須以獨立於日期選取的 Draft/Confirm 狀態機實作（R9），guard hook 與既有 Disabled Dates（R4）機制平行、互不合併，且在 Single/Range/Multi 三種選取模式下一體適用、各自獨立管理，不共用（呼應憲法 §4 時間調整、Decision 15；架構落地方式已於 2026-07-23 由 ADR-0002 拍板為併入既有 `CalendarEngine`，見第 6-7 節）。
- **使用者目標**：
  ...
  - 上層開發者可在既有 `selectedDate`/`selectedRange`/`selectedDates` 之上，為每一筆已選定日期補上時/分並合成完整 `Date`，且此調整介面提供與既有日期網格對等的無障礙/鍵盤操作保證，精度顆粒度（僅小時/小時+分鐘）可配置，不由引擎寫死（M8，落實憲法 §1 業務目的新增條、R9）。
...
- [ ] 時/分（Time-of-Day）調整核心引擎與狀態機（已選取日期 → 時間 Draft → confirm 寫入 / 中止回溯舊值），獨立於日期選取的 Draft/Confirm 機制、擁有自己獨立的 guard hook（與 R4 Disabled Dates 平行、互不合併）— 對應憲法 §1 業務目的新增條、R9、§4 時間調整、Decision 15（2026-07-22 delta，M8，架構落地方式已於 2026-07-23 由 ADR-0002 拍板，見第 6/7 節）

> **範圍消歧註記（2026-07-22 delta）**：時/分**不是** `Granularity` 的新成員，不取代、不混入日/月/季/年既有粒度定義（憲法 §1 明文）——本項與既有「粒度選取核心引擎」（M7）為兩個獨立軸，`Granularity` type 本身不因本次 delta 新增 `'time'` 成員。
...
- ❌ 多筆選取（Multi 模式）或 Range 模式起訖點共用同一時/分的批次寫入邏輯（2026-07-22 delta）— 憲法 §3 R9 明文「不屬於 engine 職責」，委派給上層應用；engine 僅保證 Range 起訖點、Multi 集合中每筆日期各自獨立的時/分 Draft/Confirm 生命週期（呼應 Decision 15 決策紀要「拒絕 Range/Multi 模式下時/分共用單一值」）
...
### Story 12: 上層開發者為已選定日期附加時/分調整（Time Adjustment，M8，2026-07-22 delta）

- **As a** 上層開發者（sanring-ui 本身，非假設性未來需求）
- **I want to** 在使用者完成日期選取後，對 `selectedDate`（或 Range 起訖點 / Multi 集合中的個別成員）補上時/分調整，並取得獨立於日期選取的 Draft/Confirm 生命週期
- **So that** 我能組裝一個「選日期 + 選時間」的完整 DateTime 選取器，且不需要自己重新設計一套 guard/中止回溯邏輯

**Acceptance Criteria**:
- [ ] Given 尚未選取任何日期，when 呼叫時間調整方法（示意用語 `updateTime()`，非最終方法名，見 Open Questions），then 呼叫被拒絕或視為 no-op（時/分無從產生 Draft，憲法 §4 時間調整「沒有日期被選取時不適用」）
- [ ] Given 已選取日期且時間分量為預設值（00:00:00 或前次 committed 值），when 呼叫時間調整方法帶入新時/分，then 進入「時間 Draft」狀態，`selectedDate` 本身的日期分量不受影響，時間分量尚未寫入（R9、§4 時間調整狀態機）
- [ ] Given 處於「時間 Draft」狀態，when 呼叫 confirm（示意用語 `confirmTime()`），then 時間分量更新為 Draft 值並視為 committed，Draft 銷毀（§4 時間調整合法轉換）
- [ ] Given 處於「時間 Draft」狀態，when 未 confirm 即中止（示意用語 `abortTimeDraft()`，或元件重新掛載等中止情境），then 時間分量回溯到 Draft 產生前的 committed 舊值，Draft 銷毀，不殘留「幽靈草稿」（比照 Decision 3「未完成交易視為從未發生」精神）
- [ ] Given guard hook 命中（precision 超出配置範圍或其他 time guard 規則，型別簽章待定），when 使用者嘗試調整時/分，then 一律靜默拒絕、時間 Draft 維持原狀不寫入（呼應 I2、R7、Decision 13 一致性原則）
- [ ] Given Range 模式已選取起訖點，when 分別對起點與終點呼叫時間調整方法，then 兩者的 Draft/Confirm 生命週期各自獨立管理，互不影響、不共用同一份 Draft 狀態（Decision 15 決策紀要）
- [ ] Given Multi 模式集合中有 N 筆已選定日期，when 對其中一筆呼叫時間調整方法，then 僅該筆日期的時/分 Draft/Confirm 狀態變動，其餘 N-1 筆不受影響（呼應 R9「不共用」）
- [ ] Given 時間調整介面透過鍵盤操作（時欄/分欄方向鍵調整數值、`Enter` confirm、`Escape` abort），then 提供與既有日期網格對等的無障礙保證（憲法 §4 時間調整備註；具體鍵盤對應表已於 2026-07-24 補齊，見第 7 節「時/分調整鍵盤互動對應表」）
...
### 時/分調整型別（Time Adjustment，R9/§4 時間調整/Decision 15，M8，2026-07-22 delta；**架構落地位置已於 2026-07-23 由 ADR-0003 拍板（supersede ADR-0002），型別命名本身仍暫定**）

```typescript
// --- 時/分調整（Time Adjustment，R9/§4 時間調整/Decision 15，M8 delta）---
// ⚠️ 下列型別命名仍為暫定,未拍板（見 Open Questions，方法命名待 owner 另行拍板）；
// 架構落地位置已拍板（見下方 ADR-0003 摘要），此處型別將實作於新檔案
// projects/date-picker/src/lib/shared/time-adjustment-state.ts。

/** 精度顆粒度；憲法明文「不得由引擎寫死單一層級」，設計為可配置 enum
 *（ADR-0002/ADR-0003 拍板：enum,非 numeric step——描述的是「哪些時間單位存在」的離散
 *  分層問題，與調整介面步進量是不同維度的參數） */
export type TimePrecision = 'hour' | 'hour-minute' | 'hour-minute-second';

/** 時/分暫定值的資料結構；是否直接用 Date 物件承載（僅時間分量有意義）
 *  或拆成獨立數值欄位，尚未拍板（見 Open Questions） */
export interface TimeValue {
  hours: number;
  minutes?: number;   // precision 為 'hour' 時可能不適用
  seconds?: number;   // precision 為 'hour-minute-second' 時才適用
}

/** guard hook 型別簽章（ADR-0002/ADR-0003 拍板：單一 predicate,不採 matcher-based——
 *  DateMatcher 測試靜態日期集合成員資格,TimeGuardMatcher 測試的是「(日期,時間)
 *  動態組合是否違反規則」,語意層級不同，硬套用 matcher-based 會牴觸 Zero Opinion 精神） */
export type TimeGuardMatcher = (date: Date, time: TimeValue) => boolean;

/** 時間調整 Draft 狀態,型別命名暫定,未拍板。呼應 §4 時間調整狀態機三態
 *（committed → Draft → confirm/abort），比照既有 Range Draft（draftStart signal）
 *  的「從未寫入穩定值,中止無需回溯邏輯」設計慣例。
 *  ADR-0003 拍板：TimeAdjustmentEngine 完全不認識 Single/Range/Multi 業務語意，
 *  targetDate 改為呼叫端自訂字串 key（見下方架構說明），baseDate 僅作為 confirm
 *  時合成年/月/日分量的不透明容器。 */
export interface TimeAdjustmentDraft {
  baseDate: Date;        // confirm 時取年/月/日分量用，engine 不查詢/依賴其來源
  draftValue: TimeValue;
}
```

**架構落地位置（2026-07-23 已由 ADR-0003 拍板，supersede ADR-0002，見 `.claude/adrs/0003-time-adjustment-engine-decoupling.md`）**：時/分調整改採**獨立 Injectable `TimeAdjustmentEngine`**（新目錄 `projects/date-picker/src/lib/time/`），完全不認識 `CalendarEngine`/`GranularityPickerEngine`，也不試圖從 `Date` 值反推 Single/Range/Multi 身份——只管理一組「呼叫方自訂字串 key → 時間 Draft」的通用儲存，Range/Multi 的角色辨識責任下放給呼叫端（預期是 `@sanring/date-picker-widget` 套件內部的組裝邏輯）。狀態轉換純函式抽在 `shared/time-adjustment-state.ts`（`startOrUpdateTimeDraft()`/`confirmTimeDraft()`/`removeTimeDraft()`，比照 ADR-0001「參數化純函式共用、狀態各自持有」慣例）。跨 engine 寫回（`confirmTimeDraft()` 拿到合成 `Date` 後呼叫 `CalendarEngine.setSelectedDate()`/`setSelectedRange()`/`setSelectedDates()`）的膠水邏輯封裝在 Widget 套件內部，裸 engine 消費者可照抄同樣兩步驟自行接線，無特權後門（符合 R5）。**`GranularityPickerEngine` 本次仍明確排除整合**（範圍僅限日粒度）；因 `TimeAdjustmentEngine` 完全不依賴 `CalendarEngine` 內部細節，未來若需要，重用成本低於 ADR-0002 原設計。

ADR-0002/ADR-0003 一併拍板的關鍵技術判斷：
1. Range 模式的角色辨識**由呼叫端傳入語意化字串 key**（如 `'range-start'`/`'range-end'`）解決，不需要引擎反推——同日 Range（起訖點恰為同一天，R8 允許 `minDays=1`）不會產生 key 碰撞，因為兩個 key 本來就是不同字串，與 `Date` 值是否相同無關。此為 ADR-0003 對 ADR-0002「需要額外角色判別參數」問題的優化解法，架構上根本消除此問題。
2. Multi 模式沿用既有 `Map<string, Date>`（§6 Multi 選取容器）正規化 key 慣例即可，但該正規化函式目前是 `CalendarEngine` 私有實作，Widget 層若要產生一致的 key 可能需要公開匯出（ADR-0003 承認的待實作技術缺口，見 Open Questions）。
3. Precision 未指定時**給合理預設值 `'hour-minute'`**，不採 Zero-default 強制注入——判斷依據是 Precision 屬「呈現層中性設定」（同 `setGridColumns()` 預設 3 的性質），非像 `CALENDAR_QUARTER_STARTS_ON` 那種「預設即可能悄悄替消費端做出錯誤業務假設」的情境。
...
### 時/分調整鍵盤互動對應表（`TimeAdjustmentEngine`，M8，2026-07-24 拍板）

> **設計原則**：時間微調元件（時欄、分欄、AM/PM 切換）在 DOM 上是獨立的可聚焦區塊，分別接收方向鍵輸入；`Enter` 對應 `confirmTimeDraft()`、`Escape` 對應 `abortTimeDraft()`，語意比照日/粒度網格的 commit/abort 慣例，消費端不需要另外設計快捷鍵層。

| 聚焦對象 | 按鍵 | 行為 |
|---|---|---|
| 時欄（hour spinner） | `↑` | `draftHours + 1`（滿 24 歸 0）；立即呼叫 `startOrUpdateTimeDraft()` 進入 pending 狀態 |
| 時欄 | `↓` | `draftHours - 1`（負則 23）；立即進入 pending 狀態 |
| 分欄（minute spinner） | `↑` | `draftMinutes + step`（`step` 預設 1，消費端可自訂；滿 60 進位到小時）；進入 pending |
| 分欄 | `↓` | `draftMinutes - step`（負則從小時借位）；進入 pending |
| AM/PM 切換鈕 | `Space` / `Enter` | 切換 meridiem（小時 ±12 mod 24）；進入 pending |
| 時欄 / 分欄 / AM/PM 切換鈕（任意一個聚焦） | `Enter` | 呼叫 `confirmTimeDraft(key)`；取得合成 `Date` 後呼叫 `CalendarEngine.setSelectedDate()`；退出 pending 狀態 |
| 時欄 / 分欄 / AM/PM 切換鈕（任意一個聚焦） | `Escape` | 呼叫 `abortTimeDraft(key)`；display 回復已確認的 `selectedDate` 時/分值；退出 pending 狀態；**不影響** `CalendarEngine` 的 `selectedDate` |
| 時欄 / 分欄（數字直接輸入） | `0`–`9` | 可選實作：累積數字輸入（例如鍵入 `1`→`4` 合成 14 時），timeout 後自動 submit 為 pending；引擎層不內建，由消費端 UI 自行決定是否實作 |

> **pending 狀態視覺提示**：消費端應於 pending 狀態（`hasDraft() === true`）以視覺標記（如邊框色變化）提示尚未確認，避免使用者誤以為時間已寫入。此為消費端責任，引擎層不感知 DOM。

### 時/分調整（Time Adjustment）Public API 草案（M8，R9/§4 時間調整/Decision 15，2026-07-22 delta；**架構落地方式已於 2026-07-23 由 ADR-0003 拍板（supersede ADR-0002），方法命名本身仍暫定**）

| 匯出符號 | 型別 | 用途 | 對應憲法條款 |
|---|---|---|---|
| `TimePrecision`（型別命名暫定） | Type | 精度顆粒度（僅小時 / 小時+分鐘 / 含秒），enum，可配置，engine 不寫死單一層級（enum,非 numeric step） | R9, §4 時間調整, Decision 15 |
| `TimeValue`（型別命名暫定） | Interface | 時/分暫定值資料結構，見第 6 節 | R9, §4 時間調整 |
| `TimeGuardMatcher`（型別命名暫定） | Type | 時間調整專屬 guard hook，與 R4 `DateMatcher` 平行、互不合併；單一 predicate 簽章（不採 matcher-based） | R9 |
| `TimeAdjustmentDraft`（型別命名暫定） | Interface | 時間 Draft 狀態 `{ baseDate, draftValue }`，見第 6 節 | R9, §4 時間調整 |
| `TimeAdjustmentEngine`（ADR-0003 拍板：**獨立 Injectable**，supersede ADR-0002 原「併入 `CalendarEngine`」方案，不整合 `GranularityPickerEngine`） | Injectable class（新增，`projects/date-picker/src/lib/time/`） | 時/分調整狀態機本體，完全不認識 `CalendarEngine`/`GranularityPickerEngine`，只管理「呼叫端自訂字串 key → 時間 Draft」 | R9, §4 時間調整, Decision 15 |

方法簽章草案（**示意用語,方法命名本身非最終,詳見憲法 Decision 15 原文與 Open Questions；掛載位置已由 ADR-0003 拍板為獨立 `TimeAdjustmentEngine`**）：

```typescript
// 以下皆為 TimeAdjustmentEngine 方法（ADR-0003 拍板掛載位置）；方法命名本身暫定,未拍板
startOrUpdateTimeDraft(key: string, baseDate: Date, time: TimeValue): void;  // 開始/更新時間 Draft；guard hook 命中則靜默 no-op（呼應 I2/R7/Decision 13）
confirmTimeDraft(key: string): Date | null;                                   // Draft → committed，回傳 baseDate 年/月/日 + time 分量合成後的完整 Date；key 不存在則回傳 null；Draft 銷毀
abortTimeDraft(key: string): void;                                            // 中止,移除該 key 的 Draft（比照 CalendarEngine.abortRangeDraft() 零回溯精神——Draft 從未寫入任何 committed 值）
setTimePrecision(precision: TimePrecision): void;                             // 可配置精度,不寫死單一層級（R9）；未指定時預設 'hour-minute'（呈現層中性設定,非強制注入）
setTimeGuard(guard: TimeGuardMatcher | undefined): void;                      // 獨立於 setDisabled()（R4）之外的另一組 guard 設定入口
```

**關鍵設計說明（ADR-0003 已拍板，supersede ADR-0002，見 `.claude/adrs/0003-time-adjustment-engine-decoupling.md`）**：`TimeAdjustmentEngine` 完全不認識 Single/Range/Multi 業務語意，`key` 由呼叫端（預期是 `@sanring/date-picker-widget`）自訂——Single 模式用固定 key（如 `'single'`）；Range 模式用語意化固定 key（如 `'range-start'`/`'range-end'`），因此不會有 ADR-0002 曾經遇到的「同日 Range key 碰撞」問題（兩個 key 本來就是不同字串，與 `Date` 值是否相同無關）；Multi 模式沿用 §6 既有 `Map<string, Date>` 正規化 key 慣例（該正規化函式目前是 `CalendarEngine` 私有實作，是否需要公開匯出供 Widget 層使用，見第 12 節 Open Questions 新增項）。`confirmTimeDraft()` 的合成邏輯由 engine 直接完成（`baseDate` 取年/月/日 + `TimeValue` 覆寫時/分/秒），呼叫端拿到完整 `Date` 後自行呼叫 `CalendarEngine.setSelectedDate()`/`setSelectedRange()`/`setSelectedDates()`（R7 既有公開方法）寫回——這段跨 engine 膠水邏輯封裝在 Widget 套件內部，裸 engine 消費者可照抄同樣兩步驟自行接線，無特權後門（符合 R5）。方法命名（`startOrUpdateTimeDraft`/`confirmTimeDraft` 等）本身仍列入第 12 節 Open Questions，留待 owner 另行拍板。
...
| ~~時/分 Draft/Confirm 機制需要在 Single/Range/Multi 三種模式下各自獨立管理（Range 起訖點各一份、Multi 集合每筆成員各一份），若架構設計不當（例如誤把時間 Draft 狀態與既有 `draftStart`/`Map<string, Date>` 選取容器耦合過深），可能複製一份「幽靈草稿」風險（違反憲法 §4 中止回溯的無殘留原則），且架構落地方式（併入既有引擎 vs 獨立模組）選錯會產生類似 Decision 12 粒度選取架構決策的高重構成本（2026-07-22 delta）~~ **已緩解（2026-07-23，ADR-0002→ADR-0003）**：最終拍板獨立 Injectable `TimeAdjustmentEngine`（完全不認識 `CalendarEngine`/`Date` 業務語意，只管理呼叫端自訂 key→Draft），跨 engine 寫回封裝在 `@sanring/date-picker-widget` 套件內部；Draft 沿用 `abortRangeDraft()` 既有「從未寫入 committed，中止免回溯」設計；「同日 Range」key 碰撞問題由呼叫端傳入語意化字串 key（`'range-start'`/`'range-end'`）從架構上根本消除，比 ADR-0002 原本額外發明角色判別參數的解法更徹底（見 ADR-0003 Alternatives Option C） | ~~high~~ low（架構已定案，剩餘風險為實作階段的測試覆蓋完整性 + Multi 模式正規化 key 是否需公開匯出的次要技術缺口） | 依 ADR-0003 Implementation Plan 執行：`time-adjustment-engine.spec.ts` 需涵蓋 guard hook 拒絕、三種 precision 合成正確性、Range 同日起訖點各用不同 key 互不碰撞、abort 零回溯四類案例；Widget 套件的膠水邏輯需另補測試 |
| Precision 可配置參數形狀（enum 如 `'hour'\|'hour-minute'\|'hour-minute-second'` vs numeric step 如「以分鐘為單位的最小刻度」）與是否強制注入（比照 `CALENDAR_QUARTER_STARTS_ON` zero-default）或提供合理預設值（比照 `setGridColumns()` 慣例）尚未定案，若選錯形狀，日後改動屬 breaking change（呼應既有 §9 風險表對 R7/R8 API churn 的既有顧慮，2026-07-22 delta） | med | M8 架構決策階段一併評估兩種形狀的消費端使用情境（sanring-ui 實際消費需求優先參考），鎖定 Public API Surface 前完成決策，避免發布後追加造成版本 churn |
| 時/分調整介面須提供與既有日期網格對等的無障礙/鍵盤操作保證（憲法明文要求），但目前既有鍵盤互動對應表（Decision 6/§7）僅涵蓋日期網格與粒度網格，尚無時間選取（如時/分數值調整用方向鍵、confirm/abort 用 Enter/Escape）的等效定義，驗收標準不明確（2026-07-22 delta） | med | M8 需新增獨立的「時間調整鍵盤互動對應表」（比照 §7 既有兩份鍵盤表格慣例），並於 Demo app 安排至少一輪人工 NVDA/VoiceOver 驗收（比照既有 M4 慣例） |
...
| M8 | 待定（架構決策已於 2026-07-23 由 ADR-0003 定案 supersede ADR-0002，排程日期仍待 owner 補，見第 12 節） | 時/分（Time-of-Day）調整，獨立 Injectable `TimeAdjustmentEngine`（ADR-0003 拍板，新目錄 `projects/date-picker/src/lib/time/`，不整合 `GranularityPickerEngine`）+ 新純函式模組 `shared/time-adjustment-state.ts`（已選取日期 → 時間 Draft → confirm 寫入 / 中止回溯，key 由呼叫端自訂）+ 獨立 guard hook（單一 predicate 簽章，與 R4 平行，不合併）+ 可配置精度（`TimePrecision` enum，預設 `'hour-minute'`，不寫死單一層級）+ Single/Range/Multi 三模式下各自獨立管理的 Draft/Confirm 生命週期（呼叫端以語意化字串 key 區分角色）+ `@sanring/date-picker-widget` 套件內的跨 engine 寫回膠水邏輯 + 時間調整介面鍵盤互動對應表（與日期網格對等的無障礙保證）— 對應憲法 §1 業務目的新增條、R9、§4 時間調整、Decision 15 | ✅ ~~架構決策先定案~~ **已完成（ADR-0003，2026-07-23，supersede ADR-0002）**；✅ §4 時間調整狀態機三態轉換測試全綠（含 confirm/abort 兩路徑，含 Single/Range/Multi 三模式，含 Range 同日起訖點以不同 key 互不碰撞）；✅ guard hook 命中時靜默拒絕、Draft 維持原狀的測試涵蓋（呼應 I2/R7/Decision 13 一致性原則測試）；✅ Range 起訖點、Multi 集合成員時/分 Draft 各自獨立、互不污染的測試涵蓋；✅ precision 可配置測試涵蓋（至少 hour / hour-minute 兩種顆粒度）；~~✅ Widget 套件跨 engine 寫回膠水邏輯測試涵蓋~~ **❌ 2026-07-26 校正：誤植/超前打勾，非真的做完**——查核 `projects/date-picker-widget/src/` 全專案，對 `TimeAdjustmentEngine`/`GranularityPickerEngine`/`selectedDates` 零程式碼引用、零測試檔，engine 側 M8 本身（狀態機/guard hook/precision/key 隔離）已如實完成，但「Widget 套件跨 engine 寫回膠水邏輯」這一項當時並未真的實作，此勾選為文件錯誤；實際交付改由 `.claude/prds/date-picker-widget.md` §10 新增的 W5-W7 里程碑負責（見該文件），本列勾選狀態自本次校正起改記為未完成，避免與 widget PRD 產生重複記帳；**2026-07-26 追記：W7（`DateTimePickerComponent`）已於同日實際完成並補上這段膠水邏輯**，但範圍僅涵蓋 Single 選取模式（Range/Multi 搭配時間調整不在內，widget PRD 標記待 owner 覆核）——本列勾選狀態維持「未完成」不變，因為本列描述的是 M8 當時的驗收門檻，不因後續 milestone 補齊而回溯改判為真，以保留稽核軌跡；~~✅ 時間調整介面鍵盤操作測試全綠~~ **❌ 2026-07-26 校正：誤植**——commit `3e6c991`（訊息「add keyboard table」）實際只在本 PRD §7 新增了「時/分調整鍵盤互動對應表」這份設計文件，`time-engine-page.component.ts/.html` 與 `time-adjustment-engine.spec.ts` 全文查核零 `(keydown)`/`ArrowUp` 等鍵盤事件綁定或對應測試，鍵盤互動表格目前只是規格，尚未落地實作；~~✅ 至少一輪人工螢幕閱讀器驗收~~ **⚠️ 2026-07-26 標記：查無佐證，待 owner 確認**——人工螢幕閱讀器驗收本質上不會留下程式碼痕跡，無法單憑 grep 判定真偽，但目前 commit 訊息/PRD/ADR 中也找不到任何記錄佐證曾執行過，與前兩項已證實誤植的勾選同批出現，一併標記供 owner 核實，不逕自認定為真或為假；demo app 新增「日期+時間」完整組裝範例 |
...
> 以下為 2026-07-22 時/分（Time-of-Day）憲法 delta（Decision 15）同步本 PRD 時新增。架構決策原於 2026-07-23 由 owner 拍板通過 ADR-0002，同日內經進一步討論後改拍板通過 **ADR-0003**（supersede ADR-0002，見 `.claude/adrs/0003-time-adjustment-engine-decoupling.md`）。guard hook/precision 相關 4 條技術判斷維持 ADR-0002 原結論不變；「架構決策」與「Range 角色辨識」兩條的結論已更新為 ADR-0003 版本；新增 1 條 ADR-0003 承認的技術缺口；其餘 4 條仍為 TODO：

- [x] 架構決策：時/分調整狀態機應併入既有 `CalendarEngine`/`GranularityPickerEngine`，或獨立 Injectable（如暫擬 `TimeAdjustmentEngine`）？→ **獨立 Injectable `TimeAdjustmentEngine`（2026-07-23 由 ADR-0003 改拍板，supersede ADR-0002 原「併入 `CalendarEngine`」結論）**：完全不認識 `CalendarEngine`/`GranularityPickerEngine`，只管理「呼叫端自訂字串 key → 時間 Draft」的通用儲存；跨 engine 寫回的膠水邏輯封裝在 `@sanring/date-picker-widget` 套件內部（owner 確認 sanring-ui 實際消費路徑透過該套件，而非繞過 Widget 直接注入裸 `CalendarEngine`，解除了 ADR-0002 拒絕獨立方案的前提）；不整合 `GranularityPickerEngine`（本次仍排除，範圍僅限日粒度）；狀態轉換邏輯抽成純函式模組 `shared/time-adjustment-state.ts`，比照 ADR-0001「參數化純函式共用、狀態各自持有」慣例。
- [x] guard hook（`TimeGuardMatcher`）型別簽章 → **單一 predicate `(date: Date, time: TimeValue) => boolean`，不採 matcher-based 設計**——`DateMatcher` 測試的是靜態日期集合成員資格，`TimeGuardMatcher` 測試的是「(日期,時間) 動態組合是否違反規則」，語意層級不同，硬套用 matcher-based 反而牴觸 Zero Opinion 精神（2026-07-23 拍板，ADR-0003 沿用 ADR-0002 原結論不變）。
- [x] Precision 參數形狀 → **enum `'hour' | 'hour-minute' | 'hour-minute-second'`，不採 numeric step**——憲法原文描述的是「哪些時間單位有意義」的離散分層問題，與 numeric step（調整介面步進量）是不同維度的參數，不互相取代（2026-07-23 拍板，ADR-0003 沿用 ADR-0002 原結論不變）。
- [x] Precision 未指定時的預設行為 → **給合理預設值 `'hour-minute'`，不採 Zero-default 強制注入**——判斷依據：Precision 是「呈現層中性設定」（性質同 `setGridColumns()` 預設 3），不是像 `CALENDAR_QUARTER_STARTS_ON` 那種「預設即可能悄悄替消費端做出錯誤業務假設」的情境，故不需強制注入（2026-07-23 拍板，ADR-0003 沿用 ADR-0002 原結論不變）。
- [x] Range 模式下如何辨識時間調整方法對應起點或終點 → **由呼叫端傳入語意化字串 key（如 `'range-start'`/`'range-end'`），不需要引擎反推身份（2026-07-23 由 ADR-0003 改拍板）**：「同日 Range」（起訖點恰為同一天，R8 允許 `minDays=1`）不會產生 key 碰撞，因為兩個 key 本來就是不同字串，與 `Date` 值是否相同無關——這比 ADR-0002 原本「需要額外角色判別參數」的解法更徹底，問題從架構上根本消失而非被繞過。
- [x] Multi 模式下時間調整方法如何定位集合中的目標成員 → **沿用既有 `Map<string, Date>`（§6 Multi 選取容器）正規化 key 慣例**，不需另外設計（2026-07-23 拍板，ADR-0003 沿用 ADR-0002 原結論不變）。
- [ ] Multi 模式的日期正規化 key 目前是 `CalendarEngine` 私有實作（新增，2026-07-23，ADR-0003 承認的技術缺口）→ 是否需要公開匯出（例如 `normalizeDayKey(date: Date): string`）供 Widget 層產生一致的 key，或由 Widget 層自行複製等效邏輯（風險：兩處實作分岔）？留待 M8 實作階段確認。
- [ ] `confirmTimeDraft()` 回傳型別設計（新增，2026-07-23，ADR-0003 提案，owner 尚未逐字拍板）→ ADR-0003 選定「`startOrUpdateTimeDraft` 多接收 `baseDate` 參數、`confirmTimeDraft` 直接回傳合成完整 `Date`」，另有「只回傳 `TimeValue`，合成交給呼叫端」的替代方案列於 ADR-0003 Alternatives Option D，待 owner review ADR-0003 全文後確認。
- [ ] 方法命名 → 示意用語 `startOrUpdateTimeDraft()`/`confirmTimeDraft()`/`abortTimeDraft()`/`setTimePrecision()`/`setTimeGuard()` 是否為最終命名，或有其他候選？**ADR-0003 明確不處理此問題**，留待 owner 另行拍板。
- [x] 時間調整介面的鍵盤互動對應表（方向鍵調整數值、Enter confirm、Escape abort 等）具體行為定義 → **已補齊（2026-07-24）**：見第 7 節「時/分調整鍵盤互動對應表」；時欄/分欄各接 `↑/↓`，AM/PM 切換鈕接 `Space/Enter`，任意時間元件聚焦下 `Enter` = `confirmTimeDraft()`、`Escape` = `abortTimeDraft()`
- [ ] 是否需要在憲法 §6 Glossary 新增「時間 Draft」「時/分」等正式術語 → 憲法本次選擇不新增，PRD 層級亦暫不新增，待日後有實際歧義再議
- [ ] M8 排程日期 → 目前留白「待定」，是否比照 M6/M7 先例先不排時程、待其他里程碑完成後直接開工，或需要現在就給一個粗略時間窗？
