...
- [ ] Given 時間調整介面透過鍵盤操作（方向鍵調整時/分數值、Enter confirm、Escape 中止），then 提供與既有日期網格對等的無障礙保證（憲法 §4 時間調整備註；具體鍵盤對應表待 M8 架構定案後補齊，見 Open Questions）
...
- [ ] 時間調整介面的鍵盤互動對應表（方向鍵調整數值、Enter confirm、Escape abort 等）具體行為定義，待 M8 實作階段一併補齊第 7 節
