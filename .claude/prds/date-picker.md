...
### 粒度選取型別（Granularity Selection，M7，2026-07-15 delta，架構決策未定見 §7）
 ...
 * 11 = 十二月。
 * TODO(owner)：此參數是否比照 CALENDAR_LOCALE 採「無預設，強制注入」
 * （I4 Zero-default 精神），或可有合理預設值（如 0 = 公曆季度）？
 * 未拍板，見 Open Questions。
 */
...
- **（2026-07-16 delta）**：`setSelectedDate()`/`setSelectedRange()`/`setSelectedDates()` 與 `setRangeDayCountLimit()` 為本次憲法 Decision 13/14 delta 新增之草案簽章，**方法命名、`setSelectedRange()`/`setSelectedDates()` 命中 disabled 時的拒絕粒度、`setRangeDayCountLimit()` 的注入機制皆未拍板**，待 owner 確認後定案（見 Open Questions）；本次不涉及既有已實作 M0-M4 方法（`selectDate()`、`setDisabled()` 等）簽章變更。

### 粒度選取（Granularity Selection）Public API 草案（M7，架構決策未定）

> **⚠️ 本節列出的符號名稱與掛載位置皆為草案**，取決於憲法 Decision 12 留給 PRD 階段的架構決策——`CalendarEngine` 擴充 `selectionGranularity` 參數，或獨立 Injectable（暫名 `GranularityPickerEngine`）——本 PRD 提出分析與傾向性建議（見下方說明），**未拍板**，待 owner 確認，必要時另開 ADR 記錄（`/supervisor:adr`）。

| 匯出符號 | 型別 | 用途 | 對應憲法條款 |
|---|---|---|---|
| `Granularity` | Type | `'day' \| 'month' \| 'quarter' \| 'year'` 選取單位判別 | R6, Decision 12 |
| `GranularityCell` | Interface | 見第 6 節；月/季/年網格最小單位 | R6, §4 粒度選取 |
| `QuarterStartMonth` | Type | 季度起始月參數型別（0=一月...11=十二月） | §4 粒度選取, Decision 12 |
| `CALENDAR_QUARTER_STARTS_ON`（暫定命名） | `InjectionToken<QuarterStartMonth>`（預設行為待定，見 Open Questions） | 季度起始月的可注入基準，獨立於 `CALENDAR_LOCALE` 之外（見第 6 節放置位置判斷） | §4 粒度選取, Decision 12 |
| `CalendarEngine.setSelectionGranularity()` **或** 新 Injectable 的等效方法（架構未定） | Method | 切換粒度（day/month/quarter/year），比照 `setSelectionMode()` 切換時一律重置選取狀態的既有慣例 | R6, §4 粒度選取, Decision 12 |
| `CalendarEngine.granularityGrids` **或** 新 Injectable 的等效輸出 signal（架構未定） | `Signal<GranularityCell[]>` | 月=12格/季=4格/年=N格（R6），非日粒度網格 | R6, §4 粒度選取, Decision 12 |

**架構決策分析（建議，非拍板）**：傾向獨立 Injectable，但建議先把 Single/Range/Multi 狀態轉換邏輯抽成與 class 解耦的純函式模組（比照現有 `calendar-grid.ts`/`calendar-disabled.ts` 慣例）供兩個 class 共用，避免程式碼重複。主要理由：

1. 型別層面已逼近拆分——`monthGrids` 型別是 `Signal<CalendarDay[][]>`，若要讓同一個 signal 同時吐出 42 格日網格與 12/4/N 格月/季/年網格，要嘛做聯集型別（消費端要自己 type narrowing，體驗差），要嘛開第二個輸出 signal；既然無論如何都要開第二個 signal，Public API Surface 的複雜度增量跟「開一個新 class」相差不大。
2. 既有 `moveFocus()` 的跨月翻頁方向鍵位移演算法（`up/down = ±7`、`left/right = ±1`）硬編碼假設 7 欄 × 6 列的日網格版面（`GRID_SIZE = 42`）。月/季/年網格版面完全不同（12/4/N 格，年粒度 N 由消費端決定），若塞進同一個 class 的同一個方法，會產生大量粒度分支條件（God Class 風險）。
3. Single/Range/Multi 的狀態轉換規則（Draft/提交/中止回溯、Set 去重、toggle 語意）憲法明講「逐條適用，只是把日期換成月/季/年」——若在獨立 Injectable 裡重寫一遍會產生高度相似但物理獨立的兩份程式碼；緩解方式是把「選取狀態轉換」抽成一組參數化純函式（以相等性比較函式為參數，日粒度傳 `isSameDay`，月/季/年粒度傳對應的 `isSameMonth`/自訂季度比較/`isSameYear`），兩個 class 各自呼叫同一組純函式。

**此分析不構成最終拍板**，證據還不足以武斷排除「單一 Engine 擴充」——若 owner 對 Public API 表面積的容忍度高於鍵盤導覽程式碼複雜度，單一 Engine 擴充仍是合理選擇。建議 M7 啟動前另開 `/supervisor:adr` 正式記錄此權衡。
...
| M7 | TODO（待 owner 排定；建議晚於 M6 完成後啟動，不與 M6 並行——理由：Decision 12「共用單一狀態機」的假設建立在 Single/Range/Multi 已於日粒度完整實作並通過測試的前提上，M6 目前程式碼尚未落地，若並行等於同時賭兩個未驗證的東西：Multi 狀態機本身的正確性、以及「這套狀態機能否乾淨地被另一個粒度重用」的抽象化假設） | 粒度選取核心引擎與狀態機（Month-picker/Year-picker/Quarter-picker，R6/§4 粒度選取/Decision 12）：架構決策定案（`CalendarEngine` 擴充 vs 獨立 Injectable，見 Open Questions/§7）+ `GranularityCell` 型別落地 + 季度起始月可注入參數 + Single/Range/Multi 三模式於新粒度下的狀態機複用 + 鍵盤導覽於非 42 格網格下的方向鍵語意（見 §9 風險） | 架構決策先定案（ADR 或 PRD 補充）；R6 三種粒度網格尺寸測試全綠；Single/Range/Multi 狀態機於 month/quarter/year 粒度下的轉換測試全綠（逐條對應日粒度既有測試）；季度起始月參數的業務歧義（公曆 vs 財年）測試涵蓋；I1-I6 於新粒度下的等效保證（無論架構選哪邊）需在測試中明確驗證 |
...
> 以下 7 條為 2026-07-15 Granularity Selection 憲法 delta（Decision 12，supersede Decision 8）同步本 PRD 時新增，皆為 TODO，尚未拍板：

- [ ] 架構決策：粒度選取是否透過 `CalendarEngine` 擴充 `selectionGranularity` 參數，或獨立 Injectable（如 `GranularityPickerEngine`）？本 PRD 給出分析與傾向性建議（見第 7 節），但未拍板，待 owner 確認，必要時另開 ADR 記錄。
- [ ] 季度起始月參數（暫定命名 `QuarterStartMonth`/`CALENDAR_QUARTER_STARTS_ON`）是否比照 `CALENDAR_LOCALE` 採「無預設，強制注入」（I4 Zero-default 精神），或可有合理預設值（如 0 = 公曆季度）？此判斷牽動 I4 詮釋範圍是否延伸至此，待 owner 拍板。
- [ ] `GranularityCell` 是否需要 per-cell 的 granularity 判別欄位（discriminant），或由消費端從呼叫的 signal/方法本身得知目前粒度即可，不需要冗餘欄位？
- [ ] 月/季/年粒度網格的鍵盤方向鍵（上下左右）版面語意如何定義？（見第 9 節風險，可能需要消費端提供欄數或下放給消費端實作）
- [ ] I1-I4/I6 不變量在新粒度下是否需要在 PRD 層級重新表述為「對任意粒度都成立」的通用形式？取決於架構決策方向（見第 7 節）。
- [ ] M7 實際排程日期 → 待 owner 排定，建議晚於 M6 完成後啟動（見第 10 節）。
- [ ] 年粒度網格 N 值的合理邊界（如 N=0 或負數時如何處理，是否需要類似 I1 的 fallback guard）？

> 以下 8 條為 2026-07-16 程式化寫值 / Range 天數限制憲法 delta（Decision 13/14）同步本 PRD 時新增，皆為 TODO，尚未拍板：

- [ ] 程式化寫值方法命名 → 暫定 `setSelectedDate()`/`setSelectedRange()`/`setSelectedDates()`，是否維持此命名，或改用其他慣例（如 `writeSelectedDate()`/`patchSelection()`）以更明顯區隔「程式化寫值」與既有互動路徑 `selectDate()`——尤其 `setSelectedDate()` 與既有唯讀輸出 signal `selectedDate` 名稱高度相似，可能造成消費端誤讀（誤以為是 signal 的 setter 而非獨立方法），待 owner 確認是否需要更明確區隔的命名。
...
- [ ] Range 天數限制參數命名 → 暫定 `RangeDayCountLimit { minDays?, maxDays? }` + `setRangeDayCountLimit()`，或改用扁平參數（如 `minRangeDays`/`maxRangeDays` 兩個獨立 setter）？待拍板。
- [ ] Range 天數限制注入機制 → component-scoped setter（比照既有 `setAllowDeselect()` 慣例）或獨立 `InjectionToken`（比照 `CALENDAR_LOCALE`/`CALENDAR_TODAY`）？待拍板，需與既有 API 慣例一致性一併考量。
- [x] Range 天數計算是否含頭尾兩端點 → **含頭尾，比照 `differenceInCalendarDays(end, start) + 1`**（2026-07-16 拍板，見第 6 節）。
- [ ] Draft 拒絕行為是否需要專屬 error signal 或事件通知消費端「為什麼被拒絕」→ 目前拒絕行為（命中 disabled 或超出天數限制）皆為靜默 no-op，消費端僅能觀察到「Draft 未提交」而無法區分拒絕原因（disabled vs 天數超限 vs 其他），是否需要新增如 `rejectionReason: Signal<'disabled' | 'day-count-limit' | null>` 之類的診斷輸出，供消費端做 UX 提示（如 toast「已超過最多可選 30 天」）？待拍板，需評估是否違背憲法 §9 Zero Opinion（引擎是否該知道「為什麼」而不只是「是否」）。
- [ ] R7 程式化寫值方法（`setSelectedDate()`/`setSelectedRange()`）是否應併入 M5（於 npm 0.1.0 鎖版前完成），或另排時程 → 待 owner 拍板（見第 10 節候選方案 A/B/C）。
